...
public class LevitationStone extends Item {
    ...
    public void inventoryTick(ItemStack itemStack, ServerLevel world, Entity entity, EquipmentSlot slot) {
        boolean isSelected = entity instanceof Player player && (player.getMainHandItem() == itemStack || player.getOffhandItem() == itemStack);
    ...
    public InteractionResult use(Level world, Player playerEntity, InteractionHand hand) {
        if(!world.isClientSide()){
            ...
            CompoundTag nbt = root.getCompoundOrEmpty("castle_in_the_sky");
            nbt.putBoolean("active", !nbt.getBooleanOr("active", false));
            ...
            if(isActive(itemStack)){
                ...
                if(blockPos!=null){
                    ...
                    CompoundTag nbt1 = root.getCompoundOrEmpty("targetLaputa");
    ...
    public void appendHoverText(ItemStack itemStack, TooltipContext context, TooltipDisplay display, Consumer<Component> tooltip, TooltipFlag iTooltipFlag) {
        ...
        tooltip.accept(Component.translatable("tooltip."+ CastleInTheSky.MOD_ID+".levitation_stone.line1").withStyle(ChatFormatting.GRAY));
        tooltip.accept(Component.translatable("tooltip."+ CastleInTheSky.MOD_ID+".levitation_stone.line2").withStyle(ChatFormatting.GRAY));
        tooltip.accept(Component.translatable("tooltip."+ CastleInTheSky.MOD_ID+".levitation_stone.line3").withStyle(ChatFormatting.GRAY));
        ...
        tooltip.accept(Component.literal(s).withStyle(ChatFormatting.GOLD));
    }

    public boolean isActive(ItemStack itemStack){
        CompoundTag nbt = getSubTag(itemStack, "castle_in_the_sky");
        return nbt.getBooleanOr("active", false);
    }

    public void spawnGuidingParticles(ItemStack itemStack, Level world, Entity entity) {
        if (!world.isClientSide() || !world.dimension().identifier().toString().equals("minecraft:overworld")) {
            return;
        }
    ...
    private static CompoundTag getSubTag(ItemStack itemStack, String key) {
        return getCustomData(itemStack).getCompoundOrEmpty(key);
    }
